(function ($) {
  ...
  jQuery(document).ready(function () {
          ...
          $.post(
            ...
            function (response) {
              ...
              $mini_cart
                ...
                .replaceWith(response.fragments.cart_subtotal);
              $mini_cart
                .find(".free-shipping-required-notice")
                .replaceWith(response.fragments.free_shipping_cart_notice);
              $thisbutton
                .closest(".widget_shopping_cart ")
                .find(".total-cart-item")
                .replaceWith(response.fragments[".total-cart-item"]);
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              $mini_cart
                ...
                .replaceWith(data.cart_subtotal);
              $mini_cart
                .find(".free-shipping-required-notice")
                .replaceWith(data.free_shipping_cart_notice);
              $thisbutton
                .closest(".widget_shopping_cart ")
                .find(".total-cart-item")
                .replaceWith(data[".total-cart-item"]);
              $cart_item.removeClass("loading");
            },
          });
          return false;
    ...
    $(document).on("added_to_cart", function (event, fragments) {
      $(".element-cart-icon").removeClass("cart-empty");
      if (
        $(".element-cart-icon:not(.off-canvas)")[0] ||
        !$(".element-cart-icon")[0]
      )
        zoo_add_to_cart_mess(fragments["zoo_add_to_cart_message"]);
    });
    $(document).on(
      "zoo_single_product_cart_added",
          ...
          if (!!data.product_id) {
            ...
            if ($("#zoo-quickview-lb")[0]) {
              $(".zoo-mask-close").removeClass(
                "mask-quick-view active loading"
              );
              ...
              success: function (response) {
                ...
                if (!$this.hasClass("zoo-buy-now")) {
                  ...
                  if ($response.find(".woocommerce-message").html()) {
                    var $shopNotices =
                      '<div id="zoo-add-to-cart-message">' +
                      $response.find(".woocommerce-message").html() +
                      "</div>";
                    if (!$(".cafe-canvas-cart")[0])
                      zoo_add_to_cart_mess($shopNotices);
                  }
                  var $cart_content = $response.find(
                    ".widget_shopping_cart_content"
                  );
                  if ($cart_content.length > 1) {
                    $cart_content = $response.find(
    ...
    $(document).on("click", ".product .btn-quick-view", function (e) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          // Variation Form
          var form_variation = $(document).find(
            "#zoo-quickview-lb .variations_form"
          );
          ...
          if ($("#zoo-quickview-lb .zoo-wishlist-button")[0]) {
            if (
              window.zooWishlist.model.exists(
                $("#zoo-quickview-lb .zoo-wishlist-button").data("id")
              )
            ) {
              window.zooWishlist.view.renderBrowseButton(
          ...
          if ($("#zoo-quickview-lb .zoo-compare-button")[0]) {
            if (
              window.zooProductsCompare.model.exists(
                $("#zoo-quickview-lb .zoo-compare-button").data("id")
              )
            ) {
              window.zooProductsCompare.view.renderBrowseButton(
                $("#zoo-quickview-lb .zoo-compare-button")
              );
            }
          }
    ...
    $(document).on(
      "click",
      ".close-quickview, .zoo-mask-close.mask-quick-view",
      function (e) {
        e.preventDefault();
    ...
    $(document).on("cleverswatch_update_gallery", function (event, response) {
      if ($("#zoo-quickview-lb")[0]) zoo_quick_view_gal();
    });
