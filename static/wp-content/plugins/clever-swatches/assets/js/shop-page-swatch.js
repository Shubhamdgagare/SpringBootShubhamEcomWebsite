(function ($) {
  ...
  $(document).ready(function () {
    $(document).on(
      "click",
      ".products .product.product-type-variable>.woocommerce-loop-product__link",
      function (event) {
        if (
          $(event.target).hasClass("zoo-cw-attr-item") ||
          $(event.target).parents(".zoo-cw-attr-item").length
        ) {
    ...
    $(document).keydown(function (event) {
      if (event.which == "17" || event.metaKey) cntrlIsPressed = true;
    });
    ...
    $(document).on(
      "click",
      ".products .product>.woocommerce-loop-product__link img:not(.zoo-cw-label),.woocommerce-loop-product__title",
      function (event) {
        let url = $(this)
          .parents(".woocommerce-loop-product__link")
          .attr("href");
        if (cntrlIsPressed) {
    ...
    function cw_processShopSelectedOption(
      ...
      if (variation_id != old_variation_id) {
        ...
        if (variation_id != 0) {
          var product_id = product_li
            .find(".zoo-cw-wrap-shop")
            .data("product_id");
          product_li.addClass("zoo-cw-loading");
          cw_updateProductImage(
            product_li,
            product_id,
            selected_options,
            variation_id
          ...
          if (allow_add_to_cart) {
            if (
              cw_checkValidOptions(product_variations, selected_options) &&
              cw_checkStock(product_variations, selected_options)
            ) {
              //add to cart process
    ...
    function cw_updateProductImage(
      product_li,
      product_id,
      selected_options,
      variation_id
          ...
          $(document).on(
            {
              mouseenter: function () {
                var swatch_wrap = $(this).parents(".zoo-cw-wrap-shop");
                var product_id = swatch_wrap.data("product_id");
                var product_li = swatch_wrap.closest(
                ...
                var allow_add_to_cart = swatch_wrap.data("allow-add-to-cart");
                if (allow_add_to_cart) {
                  var add_to_cart_button = product_li.find(
                    ".add_to_cart_button"
                  );
                  var attributes_count = swatch_wrap.data("attributes_count");
                  if (Object.keys(check_options).length == attributes_count) {
                    if (
                      cw_checkValidOptions(product_variations, check_options) &&
                      cw_checkStock(product_variations, selected_options)
                    ) {
                      $(document).trigger("cleverswatch_button_add_cart", {
                        selector: add_to_cart_button,
              ...
              mouseleave: function () {
                ...
                if (allow_add_to_cart) {
                  var product_variations =
                    swatch_wrap.data("product_variations");
                  var selected_options = swatch_wrap.data("selected_options");
                  if (selected_options === undefined) {
                    selected_options = {};
                  }
    ...
    $(document).on(
      "change",
      ".zoo-cw-group-attribute select.zoo-cw-attribute-select",
      function () {
        var swatch_wrap = $(this).parents(".zoo-cw-wrap-shop");
        var product_id = swatch_wrap.data("product_id");
        var product_li = swatch_wrap.closest(".product.post-" + product_id);
    ...
    $(document).bind(
      "cleverswatch_shop_update_image",
      function (event, response) {
        var product_li = response.selector;
        var data = response.data;
        product_li
          .find("img:not(.zoo-cw-label)")
