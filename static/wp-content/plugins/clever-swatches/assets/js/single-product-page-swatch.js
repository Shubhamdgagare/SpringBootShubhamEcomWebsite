(function ($) {
  ...
  $(document).ready(function () {
    ...
    function cw_product_gallery() {
      var icon_up =
        '<svg height="40px" tyle="enable-background:new 0 0 40 40;" version="1.1" viewBox="0 0 512 512" width="40px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><polygon points="396.6,352 416,331.3 256,160 96,331.3 115.3,352 256,201.5 "/></svg>';
      var icon_down =
        '<svg height="40px" tyle="enable-background:new 0 0 40 40;" version="1.1" viewBox="0 0 512 512" width="40px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><polygon points="396.6,160 416,180.7 256,352 96,180.7 115.3,160 256,310.5 "/></svg>';
      var icon_prev =
        '<svg height="40px" tyle="enable-background:new 0 0 40 40;" version="1.1" viewBox="0 0 512 512" width="40px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><polygon points="352,115.4 331.3,96 160,256 331.3,416 352,396.7 201.5,256 "/></svg>';
      var icon_next =
        '<svg height="40px" tyle="enable-background:new 0 0 40 40;" version="1.1" viewBox="0 0 512 512" width="40px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><polygon points="160,115.4 180.7,96 352,256 180.7,416 160,396.7 310.5,256 "/></svg>';
      if ($(".cw-product-gallery")[0]) {
        $(".cw-product-gallery .cw-product-gallery-main").slick({
          ...
          prevArrow:
            '<span class="cw-product-gallery-btn prev-item">' +
            icon_prev +
            "</span>",
          nextArrow:
            '<span class="cw-product-gallery-btn next-item ">' +
            icon_next +
            "</span>",
        });
        var columns = $(".cw-product-gallery .cw-product-gallery-thumbs").data(
          "thumb-columns"
        );
          ...
          .resize(function () {
            if (w_width != $(window).width()) {
              if (typeof $.fn.zoom != "undefined") {
                $(
                  ".cw-product-gallery .cw-product-gallery-main .woocommerce-product-gallery__image a"
    ...
    function cw_runFilter(
      ...
      attribute_rows.each(function () {
        ...
        if (attribute_display_type == "default") {
          if (disable_class == "unavailable") {
            //only run filter on click event
            //this for select type option
            $(this)
              .find("select.zoo-cw-attribute-select option")
              .each(function () {
          ...
          options.each(function () {
            ...
            check_options[attribute_name] = option_value;
            if (
              cw_findMatchVariation(product_variations, check_options, true)
            ) {
    ...
    function cw_processSelectedOption(
      product_variations,
      selected_options,
      form_add_to_cart
    ) {
      var old_variation_id = form_add_to_cart
        .find("input[name=old_variation_id]")
        .val();
      var gallery_enabled = form_add_to_cart.data("gallery_enabled");
      var add_to_cart_button = form_add_to_cart.find(
        ".single_add_to_cart_button:not(.zoo-buy-now)"
      );
      var variation_id = 0;

      var match_variation = cw_findMatchVariation(
        product_variations,
        selected_options
      );
      if (match_variation) {
      ...
      if (gallery_enabled) {
        if (variation_id != old_variation_id) {
          ...
          if (variation_id != 0) {
            ...
            add_to_cart_button
              .html(wp.template("add-to-cart-button-out-stock"))
    ...
    var wrap_class = !!zoo_cw_params.product_image_custom_class
      ? zoo_cw_params.product_image_custom_class
      : "div.woocommerce-product-gallery,div.images";
    ...
    function cw_updateGallery(
      ...
      var $gallery = $("#product-" + product_id)
        .find(wrap_class)
        .parent();
      if (
        $(".elementor-type-product.post-" + product_id + " " + wrap_class)[0]
      ) {
        $gallery = $(
          ".elementor-type-product.post-" + product_id + " " + wrap_class
        ).parent();
      } else if (
        $(".elementor.product.post-" + product_id + " " + wrap_class)[0]
      ) {
        $gallery = $(
          ".elementor.product.post-" + product_id + " " + wrap_class
        ).parent();
      }
      if ($(".cw-product-gallery")[0]) {
        $gallery = $("#product-" + product_id)
          .find(".cw-product-gallery")
          .parent();
        if (
          $(
            ".elementor-type-product.post-" +
              product_id +
              " .cw-product-gallery"
          )[0]
        ) {
          ...
          $(".elementor.product.post-" + product_id + " .cw-product-gallery")[0]
        ) {
          $gallery = $(
            ".elementor.product.post-" + product_id + " .cw-product-gallery"
          ).parent();
        }
      }
    ...
    if (!$(".variations_form.no-cw-data")[0]) {
      ...
      $(document).on(
        ...
        function () {
          ...
          var current_attribute_row = $(this).parents(
          ...
          if ($(this).hasClass("zoo-cw-active")) {
            ...
            current_attribute_row
              .parents(".zoo-cw-attr-row")
              .find(".zoo-cw-name")
              .text("");
            //process
            current_attribute_row
              .find('input[name="' + attribute_name + '"]')
              .val("");
            ...
            var options = current_attribute_row.find(
              ...
              $(document).on(
                {
                  mouseenter: function () {
                    var form_add_to_cart = $(this).parents(".variations_form");
                    var attribute_rows = form_add_to_cart.find(
                      ".zoo-cw-group-attribute"
                    );
                    var current_attribute_row = $(this).parents(
                      ".zoo-cw-group-attribute"
                    ...
                    var attribute_name =
                      current_attribute_row.data("group-attribute");
                    var product_variations =
                      form_add_to_cart.data("product_variations");
                    var option_value = $(this).data("attribute-option");
                    if (option_value != temp_option_value) {
                      temp_option_value = option_value;
                      var selected_options =
                        form_add_to_cart.data("selected_options");
                      if (selected_options === undefined) {
                        selected_options = {};
                      }
                      //visual
                      ...
                      var attributes_count =
                        form_add_to_cart.data("attributes_count");
                      if (
                        Object.keys(check_options).length == attributes_count
                      ) {
                        var match_variation = cw_findMatchVariation(
                          product_variations,
                          check_options
                        ...
                        if (match_variation) {
                          if (
                            match_variation.is_in_stock &&
                            match_variation.is_purchasable
                          ) {
                            $(document).trigger(
                              "cleverswatch_button_add_cart",
                              {
                                selector: add_to_cart_button,
                              }
                            );
                          ...
                          form_add_to_cart
                            ...
                            .html(template_html);
                          if (
                            $(".single_variation_wrap .single_variation").css(
                              "display"
                            ) == "none"
                          ) {
                            form_add_to_cart
                  ...
                  mouseleave: function () {
                    if (temp_option_value != "") {
                      ...
                      //visual
                      attribute_rows
                        .find(".zoo-cw-attribute-option")
                        .removeClass("temp-unavailable")
                        .removeClass("temp-available");
                      var selected_option_name = current_attribute_row
                        .find(".zoo-cw-attribute-option.zoo-cw-active")
                        .data("attribute-name");
                      ...
                      var attributes_count =
                        form_add_to_cart.data("attributes_count");
                      if (
                        Object.keys(selected_options).length == attributes_count
                      ) {
                        var match_variation = cw_findMatchVariation(
                          product_variations,
                          selected_options
      ...
      $(document).on(
        "change",
        ".variations_form .zoo-cw-group-attribute select.zoo-cw-attribute-select",
        function () {
          var form_add_to_cart = $(this).parents(".variations_form");
          var attribute_rows = form_add_to_cart.find(".zoo-cw-group-attribute");
          var current_attribute_row = $(this).parents(
    ...
    $(document).bind(
      "cleverswatch_update_cw_gallery",
      function (event, response) {
        var cw_gallery = $("#product-" + response.product_id).find(
          ".cw-product-gallery"
        );
        if (
          $(
    ...
    $(document).bind("cleverswatch_update_gallery", function (event, response) {
      ...
      if (
        $(
          ".elementor.product.post-" + response.product_id + " " + wrap_class
        )[0]
      ) {
        imagesDiv = $(
          ".elementor.product.post-" + response.product_id + " " + wrap_class
    ...
    $(document).bind(
      ...
      function (event, response) {
        ...
        add_to_cart_button
          .html(wp.template("add-to-cart-button-select-option"))
          .addClass("disabled wc-variation-selection-needed");
    ...
    $(document).bind(
      ...
      function (event, response) {
        ...
        var icon = add_to_cart_button.find("i").clone();
        add_to_cart_button
          .html(wp.template("add-to-cart-button"))
          .removeClass(
      ...
      if (!$(".pswp--visible")[0] && typeof PhotoSwipe !== "undefined") {
        ...
        if ($(this).parent().hasClass("slick-slide")) {
          if ($(".cw-product-gallery-main")[0]) {
            var total_sl_active = $(
              ".cw-product-gallery-main .slick-active"
            ).length;
    ...
    jQuery.fn.extend({
      ...
      cwgetGalleryItems: function () {
        var $slides = this.parents(".cw-product-gallery-main").find(
            ".cw-product-gallery-item:not(.slick-cloned)"
          ),
          items = [];
        if ($slides.length > 0) {
